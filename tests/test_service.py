...
class TestService(unittest.TestCase):
    def test_start(self):
        process = Service("Start Service Test", "test_service", "SERVICE", "sleep 0.1")
        process.start()
        self.assertEqual(process.get_state(), ServiceState.ACTIVE)
        process.stop()

    def test_stop(self):
        process = Service("Stop Service Test", "test_service", "SERVICE", "sleep 0.1")
        process.start()
        process.stop()
    ...
    def test_bad_command(self):
        process = Service(
            "Bad Service Test", "test_service", "SERVICE", "this command does not exist"
        )
